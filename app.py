...
if selected_key == "cs_shift_calendar":
    ...
    if _cs_y == 2026 and _cs_m == 6:
        ...
        _PROPOSED_MOVES = [
            ...
            ("堀田", 22, 11),
            ("堀田", 16, 15),
            # 原田 6/23 NG + 6/25 3条件 解消
            ("原田", 23, 25),
            ("金澤", 29, 23),
            # 人数バラつき抑制 (過剰日→不足日)
            ("角田", 3, 1),    # 6/1 補強 / 角田 [2-6]→[1-2]+[4-6] 連勤分断
            ("雨貝", 4, 1),    # 6/1 補強 / 雨貝 [2-6]→ 分断
            ("菊地", 18, 14),  # 6/14 バクダン解消 / 菊地 [15-19]→ 分断
            ("角田", 11, 15),  # 6/15 補強 / 角田 [8-12]→[8-10]+[12]
            ("雨貝", 3, 7),    # 6/7 補強 / 雨貝 [2-6]→[1-2]+[5-7] 大幅改善
            ("菊地", 17, 20),  # 6/20 補強 / 菊地 [15-19]→ 分断
            ("角田", 22, 14),  # 6/14 補強 / 角田 [17-22]6連→[17-21]5連
            ("葛西", 2, 1),    # 6/1 バクダン解消(6→7) / 葛西 [2-6]5連→[3-6]4連 同時改善
        ]
