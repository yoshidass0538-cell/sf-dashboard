...
if selected_key.startswith("talk_script_"):
    ...
    for sec_name in sections:
        ...
        _cur_board_label = _parsed_title[1] if _parsed_title else ""
        if sec_name == "不備解消" and kind == "Sonet" and _cur_board_label != "1週間後FCトーク":
...
if selected_key == "day_calls":
    ...
    def _render_bar_chart(title: str, df_src):
        ...
        _status_colors = {"開通前対応": "#1e88e5", "開通後対応": "#fb8c00"}
        _palette = [
            "#43a047", "#8e24aa", "#00acc1", "#e53935", "#ec407a", "#00897b",
            "#7cb342", "#5e35b1", "#d81b60", "#546e7a", "#6d4c41", "#c2185b",
        ]
...
if selected_key == "ccr":
    ...
    _BREAKS = [(670, 680), (730, 740), (790, 800), (840, 900),
               (970, 980), (1030, 1040), (1090, 1100), (1150, 1160)]
    ...
    for _i, _p in enumerate(_ccr_people):
        ...
        with _cols[_i % _ncol]:
            st.markdown(
                ...
                f'font-size:11px;font-weight:700;line-height:20px;text-align:center;">'
                f'<div style="width:{_tw:.1f}%;background:#43a047;color:#fff;overflow:hidden;">'
    ...
    st.markdown(
        ...
        '<span style="color:#7a8a99;">（休憩=10分休憩×7（11:10/12:10/13:10/16:10/17:10/18:10/19:10）＋昼休憩14:00-15:00。本日シフトのある稼働メンバーのみ表示）</span><br>'
...
if selected_key == "kpi_call_target":
    ...
    try:
        _ki = _load_kpi_indiv("ActivityDate = LAST_N_DAYS:30")
    except Exception as _e:
    ...
    def _rate_cell(rate):
        # 対象種別は全架電/FCの約5割が標準のため、充足率も40〜50%が目安。45/25 で相対表示
        bg = "#c8e6c9" if rate >= 45 else ("#fff3c4" if rate >= 25 else "#ffcdd2")
    ...
    def _render_indiv_summary(ki):
        ...
        st.markdown(
            f'<div style="overflow-x:auto;"><table class="kpi-tbl">{ah}{ab}</table></div>',
            unsafe_allow_html=True,
        )
    ...
    def _render_indiv_daily(ki):
        ...
        for m in ki["members"]:
            ...
            st.markdown(
                f'<div style="overflow-x:auto;"><table class="kpi-tbl">{dh}{db}</table></div>',
                unsafe_allow_html=True,
            )
        if not any_shown:
            st.info("この月の対象コールはありません。")
    ...
    for _i in range(3):
        _months.append((_yy, _mm))
        _mm -= 1
        if _mm == 0:
            _mm = 12
            _yy -= 1
    _mlabels = [f"{y}年{m}月" + ("（当月）" if i == 0 else "") for i, (y, m) in enumerate(_months)]
    _mtabs = st.tabs(_mlabels)
    for _tab, (_y, _m) in zip(_mtabs, _months):
        with _tab:
            _last = _kpi_cal.monthrange(_y, _m)[1]
            _mf = f"ActivityDate >= {_y}-{_m:02d}-01 AND ActivityDate <= {_y}-{_m:02d}-{_last:02d}"
            try:
                _kim = _load_kpi_indiv(_mf)
            except Exception as _e:
                st.error(f"{_y}年{_m}月の集計に失敗しました: {_e}")
                continue
            _render_indiv_daily(_kim)
    st.stop()
