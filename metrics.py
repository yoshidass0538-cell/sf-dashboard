...
METRICS: list[Metric] = [
    ...
    Metric(
        key="call_type_total",
        label="架電種別トータルコール数",
        description="商材別(全て/NIFTY/NURO/SO-NET/AU光)×日別×担当者別に、架電種別ごとのコール結果件数・合計・割合を集計",
        fetch=lambda sf: pd.DataFrame(),
        category="TOTAL",
    ),
    Metric(
    ...
    Metric(
        key="fubitaitai_kirisute_area",
        label="不備停滞 切り捨て判定資料(エリア別)",
        description="ソネット光の不備停滞理由別集計を「全件/東日本/西日本」のタブで切替。経験ベース・直近3ヶ月除外",
        fetch=lambda sf: pd.DataFrame(),
        category="資料",
    ),
    Metric(
        key="fubitaitai_kirisute_list",
        label="不備停滞 切り捨て判定資料(リスト別)",
        description="ソネット光の不備停滞理由別集計を「AUリスト/ドコモリスト/SBリスト」のタブで切替。利用携帯ⅠⅡで分類・直近3ヶ月除外",
        fetch=lambda sf: pd.DataFrame(),
        category="資料",
    ),
    Metric(
